...
def submit_data():
    ...
    activity_info = activity_entry.get()
    ...
    response = co.classify(model='large', inputs=inputs, examples=examples)
    ...
    if predictions:
        ...
        matched_row = df[df['category_name'] == max_prediction.prediction]
        ...
        if len(matched_row) > 0:
            ...
            Hm = call.Phone()
            Hm.dial(person_name, max_prediction.prediction, business_name, activity_time, activity_info, phone_number)
        else:
...
# Entry field for "Activity and Location"
activity_label = ttk.Label(frame, text="Prompt:")
...
activity_time_label = ttk.Label(frame, text="Booking Time:")
...
submit_button.grid(row=6, columnspan=2, pady=(20, 0))
